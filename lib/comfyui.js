...
// ---------------------------------------------------------------------------
// img2img: upload input image to ComfyUI then run KSampler with denoise < 1
// ---------------------------------------------------------------------------
async function uploadImageToComfyUI(imagePath) {
  // ComfyUI /upload/image endpoint accepts multipart/form-data
  ...
  if (!result.name) throw new Error(`Upload failed: ${JSON.stringify(result)}`);
  console.log(`[comfyui] uploaded input image as: ${result.name}`);
  return result.name;
}

function buildImg2ImgWorkflow(uploadedName, prompt, options = {}) {
  const { denoise = 0.6, steps = 20, style = '' } = options;
  const fullPrompt = style ? `${style}, ${prompt}` : prompt;
  const seed = Math.floor(Math.random() * 1e15);

  // Node map:
  //  4  = CheckpointLoaderSimple (base model)
  //  20 = LoadImage (input image)
  //  21 = VAEEncode
  //  6  = positive CLIPTextEncode
  //  7  = negative CLIPTextEncode
  //  3  = KSampler (img2img, denoise < 1)
  //  8  = VAEDecode
  //  9  = SaveImage
  return {
    '4': {
    ...
    '9': {
      class_type: 'SaveImage',
      inputs: { filename_prefix: 'jarvis-i2i', images: ['8', 0] },
    },
  };
}

async function generateImg2Img(imagePath, prompt, options = {}, chatId, bot, sendToTelegram) {
  const enhanced = await enhancePrompt(prompt);

  let uploadedName;
  try {
    uploadedName = await uploadImageToComfyUI(imagePath);
  } catch (err) {
    if (err.message === 'OFFLINE' || err.code === 'ECONNREFUSED') {
      await sendToTelegram(OFFLINE_MSG);
      return null;
    }
    await sendToTelegram(`Upload failed: ${err.message}`);
    return null;
  }

  const workflow = buildImg2ImgWorkflow(uploadedName, enhanced, options);
  const payload = JSON.stringify({ prompt: workflow });
  ...
  const promptId = submitResult.prompt_id;
  if (!promptId) {
    await sendToTelegram(`ComfyUI rejected img2img: ${JSON.stringify(submitResult)}`);
    return null;
  }

  console.log(`[comfyui] img2img prompt_id=${promptId}`);
  ...
  for (let i = 0; i < 150; i++) {
    ...
    for (const nodeId of Object.keys(outputs)) {
      ...
      const { filename, subfolder = '', type = 'output' } = images[0];
      const imgBuffer = await httpRequest({
        host: COMFYUI_HOST,
        port: COMFYUI_PORT,
        path: `/view?filename=${encodeURIComponent(filename)}&subfolder=${encodeURIComponent(subfolder)}&type=${encodeURIComponent(type)}`,
        method: 'GET',
      ...
      console.log(`[comfyui] img2img buffer size: ${imgBuffer.length} bytes`);
      await bot.sendPhoto(chatId, imgBuffer, { caption: 'Here you go, Boss.' });
      return imgBuffer;
    }
  }

  await sendToTelegram('img2img timed out — ComfyUI took too long.');
  return null;
}

module.exports = { generateImage, generateForTelegram, generateImg2Img };
